...
Class FHT
    ...
    Private Sub FHTinit(ByVal nn As Int32, ByVal m_WindowType As WindowTypes)
        ...
        For i As Int32 = 0 To nn - 1
            ...
            Select Case m_WindowType
                ...
                Case WindowTypes.Sin4
                    hTab(i) = Pow(Sin(ang / 2), 4) * knn * 2.15
                Case WindowTypes.Sin8
                    hTab(i) = Pow(Sin(ang / 2), 8) * knn * 2.85
                Case WindowTypes.Sin12
                    hTab(i) = Pow(Sin(ang / 2), 12) * knn * 3.44
                Case WindowTypes.Sin16
                    hTab(i) = Pow(Sin(ang / 2), 16) * knn * 3.9
                Case WindowTypes.Sin24
                    hTab(i) = Pow(Sin(ang / 2), 24) * knn * 4.75
    ...
    Friend Sub ExecuteFHT(ByVal InputBuffer() As Int16, _
                          Optional ByVal WindowType As FHT.WindowTypes = WindowTypes.Sin12)
        '
        ...
        For i = 0 To nn - 1
            aTab(permTab(i)) = InputBuffer(i) * hTab(i)
        Next
        ' --------------------------------------------------------------------- FHT
        le = 1
        trig_inc = nn >> 1
